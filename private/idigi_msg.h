...
static void msg_set_error(msg_session_t * const session, idigi_msg_error_t const error_code)
{
    ...
    idigi_bool_t const client_request_error = MsgIsTrue(MsgIsClientOwned(dblock->status_flag) && !MsgIsReceiving(dblock->status_flag));
    idigi_bool_t const client_response_error = MsgIsTrue(!MsgIsClientOwned(dblock->status_flag) && !MsgIsReceiving(dblock->status_flag));
    idigi_bool_t const server_request_error = MsgIsTrue(!MsgIsClientOwned(dblock->status_flag) && MsgIsReceiving(dblock->status_flag));
    ...
    if (client_request_error && MsgIsStart(dblock->status_flag))
    {
        ...
        dblock->state = msg_state_delete;
        goto done;
    }

    session->error = error_code;
    dblock->state = msg_state_send_error;
                                          ...
                                          idigi_bool_t const client_owned, idigi_msg_error_t * const result)
{
    ...
    *result = idigi_msg_error_memory;
    ...
    case msg_service_id_rci:
        double_buffer = idigi_true;
        break;

    default:
        double_buffer = idigi_false;
        break;
    }
    ...
    if (session->out_dblock != NULL) 
    {
        ...
        session->out_dblock->state = msg_state_init;
    }

    if (session->in_dblock != NULL) 
    {
        session->in_dblock->status_flag = status;
        session->in_dblock->state = msg_state_init;
    }
...
static idigi_msg_error_t msg_initialize_data_block(msg_session_t * const session, uint32_t const window_size, msg_block_state_t state)
{
    ...
    case msg_block_state_send_request:
        ...
        session->out_dblock->state = msg_state_get_data;
        #if (defined IDIGI_COMPRESSION)
    ...
    case msg_block_state_recv_request:
        ...
        session->in_dblock->state = msg_state_receive;
        #if (defined IDIGI_COMPRESSION)
...
static void msg_send_complete(idigi_data_t * const idigi_ptr, uint8_t const * const packet, idigi_status_t const status, void * const user_data)
{
...

    {
        ...
        {
            dblock->state = MsgIsRequest(dblock->status_flag) ? msg_state_receive : msg_state_delete;
            goto done;
        }

        if (dblock->zlib.avail_out == 0)
        {
            dblock->state = msg_state_compress;
            goto done;
        }

        dblock->z_flag = Z_NO_FLUSH;
        dblock->state = MsgIsAckPending(dblock->status_flag) ? msg_state_wait_ack : msg_state_get_data;
...
static idigi_callback_status_t msg_send_compressed_data(idigi_data_t * const idigi_ptr, msg_session_t * const session)
{
    ...
    case idigi_callback_busy:
        dblock->state = msg_state_send_compressed;
        break;

    case idigi_callback_continue:
        dblock->bytes_out = 0;
        dblock->state = msg_state_wait_send_complete;
        break;
...
static idigi_callback_status_t msg_get_service_data(idigi_data_t * const idigi_ptr, msg_session_t * const session)
{
...

    {
        ...
        service_data->length_in_bytes = available_bytes;
        service_data->flags = flag;
        status = msg_call_service_layer(idigi_ptr, session, msg_service_type_need_data);
        if (status != idigi_callback_continue)
            goto error;
    ...
    {
        MsgSetAckPending(dblock->status_flag);
        dblock->state = msg_state_wait_ack;
    }
...
static idigi_callback_status_t msg_send_ack(idigi_data_t * const idigi_ptr, idigi_msg_data_t * const msg_ptr, msg_session_t * const session)
{
    ...
    if (status != idigi_callback_busy)
    {
        ...
        dblock->state = msg_state_receive;
    }
...
static idigi_callback_status_t msg_pass_service_data(idigi_data_t * const idigi_ptr, msg_session_t * const session, void * data, size_t const bytes)
{
    ...
    if (status == idigi_callback_continue)
    {
        ...
        else
        {
            ...
            {
                MsgSetAckPending(dblock->status_flag);
                dblock->state = msg_state_send_ack;
            }
            else
                dblock->state = msg_state_receive;
        }
...
static idigi_callback_status_t msg_process_decompressed_data(idigi_data_t * const idigi_ptr, msg_session_t * const session)
{
    ...
    case idigi_callback_continue:
        dblock->bytes_out = 0;
        if (dblock->state != msg_state_get_data)
        {
            if (dblock->zlib.avail_out == 0)
                dblock->state = msg_state_decompress;
            else
            {
    ...
    case idigi_callback_busy:
        dblock->state = msg_state_process_decompressed;
...
static idigi_callback_status_t msg_decompress_data(idigi_data_t * const idigi_ptr, msg_session_t * const session)
{
...

    {
        ...
        dblock->state = MsgIsAckPending(dblock->status_flag) ? msg_state_send_ack : msg_state_receive;
        switch(zret) 
        {
        case Z_BUF_ERROR:
...
static idigi_callback_status_t msg_process_service_data(idigi_data_t * const idigi_ptr, msg_session_t * const session, uint8_t * msg_data, size_t const frame_bytes, size_t const header_bytes, unsigned const flag)
{
    ...
    ASSERT_GOTO(dblock != NULL, error);
    if (dblock->state != msg_state_receive)
    {
        status = ((dblock->state == msg_state_send_error) || (dblock->state == msg_state_delete)) ? idigi_callback_continue : idigi_callback_busy;
        goto done;
    }
...
static idigi_callback_status_t msg_process_start(idigi_data_t * const idigi_ptr, idigi_msg_data_t * const msg_ptr, uint8_t * ptr, uint16_t const length)
{
    ...
    {
        if ((session->out_dblock != NULL) && (session->out_dblock->state == msg_state_send_error))
            goto done;
    }
...
static idigi_callback_status_t msg_process_ack(idigi_msg_data_t * const msg_fac, uint8_t const * ptr)
{
...

    {
        ...
        {
            MsgClearAckPending(dblock->status_flag);
            if (dblock->state == msg_state_wait_ack)
                dblock->state = msg_state_get_data;
        }
    }
...
static idigi_callback_status_t msg_process_error(idigi_data_t * const idigi_ptr, idigi_msg_data_t * const msg_fac, uint8_t * const ptr)
{
    ...
    if (session != NULL)
    {
        msg_data_block_t * const dblock = (session->out_dblock != NULL) ? session->out_dblock : session->in_dblock;

        ASSERT_GOTO(dblock != NULL, error);
        if ((dblock->state != msg_state_delete) && (dblock->state != msg_state_send_error))
        {
