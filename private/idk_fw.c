...
#define FW_TARGET_LIST_MSG_INTERVAL_PER_SECOND     (30 * MILLISECONDS_PER_SECOND)
...
static uint32_t get_timeout_limit(uint32_t max_timeout, uint32_t previous_timeout)
{
    uint32_t time_limit;

    time_limit = (max_timeout * MILLISECONDS_PER_SECOND) - previous_timeout;
    return time_limit;
}
                                           ...
                                           void * response, size_t * response_size, fw_compare_type_t compare_type)
{
    ...
    rx_keepalive = get_timeout_limit(*idk_ptr->rx_keepalive, (time_stamp - idk_ptr->rx_ka_time));
    tx_keepalive = get_timeout_limit(*idk_ptr->tx_keepalive, (time_stamp - idk_ptr->tx_ka_time));
    ...
    if (fw_ptr->ka_time > 0)
    {
         ...
         */
        timeout = MIN_VALUE( timeout, FW_TARGET_LIST_MSG_INTERVAL_PER_SECOND - (time_stamp - fw_ptr->ka_time));
    ...
    if (request != NULL)
    {
        ...
        *req_timeout = timeout/MILLISECONDS_PER_SECOND;
    }
    ...
    if (status == idk_callback_busy && fw_ptr->ka_time > 0)
    {
         ...
         */
        fw_ptr->keepalive = ((time_stamp1 - fw_ptr->ka_time) >= FW_TARGET_LIST_MSG_INTERVAL_PER_SECOND);
    }
    else
    {
...
static fw_abort_status_t get_abort_status_code(idk_fw_status_t status)
{
    fw_abort_status_t code = fw_user_abort;

    switch (status)
    {
    case idk_fw_user_abort:
        code = fw_user_abort;
        break;
    case idk_fw_device_error:
        code = fw_device_error;
        break;
    case idk_fw_invalid_offset:
        code = fw_invalid_offset;
        break;
    case idk_fw_invalid_data:
        code = fw_invalid_data;
        break;
    case idk_fw_hardware_error:
        code = fw_hardware_error;
        break;
    case idk_fw_success:
    case idk_fw_download_denied:
    case idk_fw_download_invalid_size:
    case idk_fw_download_invalid_version:
    case idk_fw_download_unauthenticated:
    case idk_fw_download_not_allowed:
    case idk_fw_download_reject:
    case  idk_fw_encountered_error:
        ASSERT(false);
        break;
...
#define FW_SEND_ABORT_LENGTH    3
    ...
    if (msg_opcode != fw_error_opcode && (status < idk_fw_user_abort || status > idk_fw_hardware_error))
    {
...
#define FW_INFO_REQUEST_ID_COUNT    4
    ...
    while (fw_ptr->request_id < FW_INFO_REQUEST_ID_COUNT)
    {
        ...
        case idk_firmware_version:
        {
            ...
            status = get_fw_config(fw_ptr, idk_firmware_version, &request, sizeof request,&fw_ptr->version, NULL, fw_equal);
            break;
        }
        case idk_firmware_code_size:
        {
            idk_fw_config_t request;
            request.target = target;
            status = get_fw_config(fw_ptr, idk_firmware_code_size, &request, sizeof request, &fw_ptr->code_size, NULL, fw_equal);
            break;
        }
        ...
        case idk_firmware_name_spec:
            if (request_list[fw_ptr->request_id] == idk_firmware_description)
            {
                ...
                request.target = target;
                status = get_fw_config(fw_ptr, idk_firmware_description, &request, sizeof request, (void *)&fw_ptr->description, &fw_ptr->desc_length, fw_less_than);
            }
            else
            {
                ...
                request.target = target;
                status = get_fw_config(fw_ptr, idk_firmware_name_spec, &request, sizeof request, (void *)&fw_ptr->name_spec, &fw_ptr->spec_length, fw_less_than);
            }

            if (status == idk_callback_continue && (fw_ptr->desc_length + fw_ptr->spec_length) > (FW_ID_STRING_LENGTH -1))
            {
...
static idk_callback_status_t process_fw_download_request(idk_firmware_data_t * fw_ptr, idk_facility_packet_t * packet)
{
    ...
    if (status != idk_callback_busy)
    {
        ...
        {
            send_fw_abort(idk_ptr, request_data.target, fw_download_abort_opcode, response_status);
            goto done;
        }
...
static idk_callback_status_t process_fw_complete(idk_firmware_data_t * fw_ptr, idk_facility_packet_t * packet)
{
    ...
    {
        send_fw_abort(idk_ptr, request_data.target,fw_download_abort_opcode, response_data.status);
    }
...
static idk_callback_status_t fw_process(idk_data_t * idk_ptr, idk_facility_t * fac_ptr)
{
    ...
    uint8_t * ptr;
    idk_firmware_data_t * fw_ptr = GET_FACILITY_POINTER(fac_ptr);
...
static idk_callback_status_t fw_discovery(idk_data_t *idk_ptr, idk_facility_t * fac_ptr)
{
    ...
    if (fw_ptr->request_id == idk_firmware_target_count)
    {
        ...
        status = get_fw_config(fw_ptr, idk_firmware_target_count, &timeout, sizeof timeout, &fw_ptr->target_count, NULL, fw_equal);
        if (status == idk_callback_continue)
        {
            packet->length = 0;
...
{
    return del_facility_data(idk_ptr, E_MSG_FAC_FW_NUM);
}
...
static idk_callback_status_t fw_init_facility(idk_data_t *idk_ptr)
{
    ...
    fw_ptr = get_facility_data(idk_ptr, E_MSG_FAC_FW_NUM);
    if (fw_ptr == NULL)
    {
        void * ptr;
        status = add_facility_data(idk_ptr, E_MSG_FAC_FW_NUM, &ptr,
        ...
        fw_ptr = (idk_firmware_data_t *)ptr;
   }
    fw_ptr->request_id = 0;
