...
def get_template_dirs():
    ...
    idigi_config_path = os.path.join(sandbox_dir, 'public/include/idigi_config.h')

    return (sandbox_dir, template_script_dir, template_platform_dir, template_test_dir, idigi_config_path)

def build_template():
    (sandbox_dir, template_script_dir, template_platform_dir, template_test_dir, idigi_config_path) = get_template_dirs()
    ...
    (sandbox_dir, template_script_dir, template_platform_dir, template_test_dir, idigi_config_path) = get_template_dirs()
    setup_platform(config, template_script_dir, template_platform_dir, '000000:000000')
    config.replace_string(idigi_config_path, 'IDIGI_COMPRESSION', 'IDIGI_NO_COMPRESSION')
    config.replace_string(idigi_config_path, 'IDIGI_FIRMWARE_SERVICE', 'IDIGI_NO_FIRMWARE_SERVICE')
    build_test(TEMPLATE_TEST_DIR)
    shutil.rmtree(sandbox_dir)

    (sandbox_dir, template_script_dir, template_platform_dir, template_test_dir, idigi_config_path) = get_template_dirs()
    setup_platform(config, template_script_dir, template_platform_dir, '000000:000000')
    config.replace_string(idigi_config_path, 'IDIGI_DATA_SERVICE', 'IDIGI_NO_DATA_SERVICE')
    build_test(TEMPLATE_TEST_DIR)
    shutil.rmtree(sandbox_dir)

    (sandbox_dir, template_script_dir, template_platform_dir, template_test_dir, idigi_config_path) = get_template_dirs()
    setup_platform(config, template_script_dir, template_platform_dir, '000000:000000')
    config.replace_string(idigi_config_path, 'IDIGI_NO_FIRMWARE_SERVICE', 'IDIGI_FIRMWARE_SERVICE')
    build_test(TEMPLATE_TEST_DIR)
    shutil.rmtree(sandbox_dir)
