...
static firmware_list_t firmware_list[] = {
    ...
    {0x00000000, -1, ".*\\.*",        "any Image"}, /* any *.a files */
};
...
{
    idigi_callback_status_t status = idigi_callback_continue;
    idigi_status_t idigi_status;

    UNUSED_ARGUMENT(reset_data);
    APP_DEBUG("firmware_reset\n");

    idigi_status = idigi_initiate_action(idigi_handle, idigi_initiate_terminate, NULL, NULL);
    if (idigi_status != idigi_success)
    {
        APP_DEBUG("app_firmware_reset: idigi_initiate_terminate error %d\n", idigi_status);
    }
