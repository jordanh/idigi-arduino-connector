...
def send_rci(request, url=SERVER_URL, 
             username=SERVER_USERNAME,
             password=SERVER_PASSWORD):
    #Used if we receive bad XML from iDigi/Device
    sleep(RCI_SLEEP)
    response = post(SERVER_URL, data=request, 
...
def test_device_rci():
    request = [{'query':RCI_DESC_TEMPLATE.substitute(request=QUERY_DESCRIPTOR_SETTING),'isState':False},
               {'query':RCI_DESC_TEMPLATE.substitute(request=QUERY_DESCRIPTOR_STATE),'isState':True},
               ]
    for r in request:
        isState = r['isState']
        rci=r['query']
