...
/******************************************************************************
 * Function:        int MACInit(waitComplete)
 *
 * PreCondition:    None
 *
 * Input:           waitComplete - compatibility with PIC32MX7 family.
 *                                 Not used.
 *
...
int MACInit(int waitComplete)
{
	...
	ChangeSPIBRG(0xFF);                                     // run really slow
    ...
    ChangeSPIBRG(SPICalutateBRG(GetPeripheralClock(), 20000000));
...
{
	return Que_IsQueued( &MACTxQueue, sbfr );
}

/////////////////////////////////////////////////
// return 1 if link is ON, 0 otherwise
/////////////////////////////////////////////////
...
NET_PKT* pPktBfr;
    ...
    if( !MACTxQueue.PktCount )
		return 1;		//nothing in the queue, so just return.
    ...
    // Link is up and Tx Queue not empty - send packets out if chip ready

	while( !ReadETHReg(ECON1).ECON1bits.TXRTS ) // Make sure the transmitter is ready
	{
...
NET_PKT* MACGetRxPacket(void)
{
	...
	BankSel(ERDPTL);
	WriteReg(ERDPTL, NextRxAddr.v[0]);
	WriteReg(ERDPTH, NextRxAddr.v[1]);
...
//	       PktPreamble.StatusVector.bits.ByteCount > MAX_ETHERNET_FRAME_SZ)
	...
	pNetPkt = (NET_PKT*)AllocPacketBuffer( PktPreamble.StatusVector.bits.ByteCount, 0, PKT_FLAG_RX );
	if( pNetPkt )
	{
		//save packet size
		...
		MACGetArray( (BYTE *)pNetPkt->pPktLoad, pNetPkt->PktLen );
	}
	else
	{
	...
	// high byte last.
	BankSel(ERXRDPTL);
	WriteReg(ERXRDPTL, next_location.v[0]);
	WriteReg(ERXRDPTH, next_location.v[1]);
...
static BOOL XmitPkt( NET_PKT* sbfr )
{
    ...
    MACPutArray( (BYTE *)sbfr->pPktLoad, sbfr->PktLen );
