...
#define IDIGI_MAC_ADDR_LENGTH     6
...
static bool get_ip_address(uint8_t ** ip_address, size_t *size)
{
	...
	if (hostent_addr == NULL)
	{
		int error = WSAGetLastError();
		APP_DEBUG("get_ip_address: gethostbyname failed %d\n", error);
		goto done;
	}

	{
		char **ptr;
		for (ptr = hostent_addr->h_aliases; *ptr != NULL; ptr++)
		{
			APP_DEBUG("Alternate name: %s\n", *ptr);
		}

		switch (hostent_addr->h_addrtype)
		{
		case AF_INET:
		{
			int i;
			APP_DEBUG("AF_INET address\n");
			i = 0;
			
			while (hostent_addr->h_addr_list[i] != 0)
			{
				struct in_addr addr;
				*ip_address = (uint8_t *)hostent_addr->h_addr_list[i];
				*size = sizeof *(u_long *)hostent_addr->h_addr_list[i];
				addr.s_addr = *(u_long *)hostent_addr->h_addr_list[i];
	            APP_DEBUG("\tIP Address #%d: %s\n", i, inet_ntoa(addr));
				i++;
				result = true;
			}
			break;
		...
		case AF_NETBIOS:
			APP_DEBUG("AF_NETBIOS address\n");
			break;
		default:
			APP_DEBUG("%d address type\n", hostent_addr->h_addrtype);
			break;
		}
	}
done:
...
static uint8_t device_mac_addr[IDIGI_MAC_ADDR_LENGTH] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
...
{
    static uint8_t device_id[IDIGI_DEVICE_ID_LENGTH] = {0};
...
// #error  "Specify vendor id"
    static const uint8_t device_vendor_id[IDIGI_VENDOR_ID_LENGTH] = {0x00, 0x00, 0x00, 0x00};
...
// #error "Specify device type"
    static char const * const device_type = "Linux Application";
