...
iDigiDataset dataset(4, "arduino");    // iDigiDiaDataset objects store our samples
...
void loop() {
  ...
  if (idigi_connected && (now - next_upload_time >= 0) && dataset.size() > 0)
  {
    ...
    if (bytesUploaded < 0)
    {
      Serial.print("sendDataSet: error returned from sendDataSet(");
      Serial.print(bytesUploaded, DEC);
      Serial.println(")");
      
      return;
    } else {
      Serial.print("sendDataSet: uploaded ");
      Serial.print(bytesUploaded, DEC);
