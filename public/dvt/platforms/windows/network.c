...
static bool dns_resolve_name(char const * const domain_name, unsigned long * ip_addr)
{
    bool rc=false;
...
static idigi_callback_status_t network_connect(char const * const host_name, size_t const length, idigi_network_handle_t ** network_handle)
{
    ...
    if ((rc == idigi_callback_abort) && (fd != INVALID_SOCKET))
    {
        closesocket(fd);
        fd = INVALID_SOCKET;
    }
...
static idigi_callback_status_t network_close(idigi_network_handle_t * const fd)
{
	...
	{
		APP_DEBUG("network_close: shutdown failed %d\n", WSAGetLastError());
		closesocket(*fd);
	}
