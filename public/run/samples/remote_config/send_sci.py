...
# Send SCI request.
# -------------------------------------------------
# Usage: send_sci.py <username> <password> <device_id> <request file>
# -------------------------------------------------
...
def Usage():
    print 'Usage: send_sci.py <username> <password> <device_id> <request_file>'
    print '       It sends SCI request_file request message to the server\n'
   
def PostMessage(username, password, device_id, request_file):
    ...
    try: 
        fileHandle = open(request_file, 'r')
    except IOError:
...
def main(argv):
    ...
    if count != 4:
        ...
        PostMessage(argv[0], argv[1], argv[2], argv[3])
