...
#error  "Specify device id"
    ...
    size_t mac_size;
    idigi_callback_status_t  ret = idigi_callback_abort;

    /* This sample uses the MAC address to format the device ID */
    ret = app_get_mac_addr(&mac_addr, &mac_size);
...
static idigi_callback_status_t app_get_vendor_id(uint8_t const ** id, size_t * const const size)
{
