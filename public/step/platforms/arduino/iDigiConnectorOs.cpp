 ...
 */
int iDigiConnectorClass::app_os_malloc(size_t const size, void ** ptr)
{
    int status=-1;
 ...
 */
void iDigiConnectorClass::app_os_free(void * const ptr)
{
    if (ptr != NULL)
    {
        free(ptr);
 ...
 */
int iDigiConnectorClass::app_os_get_system_time(unsigned long * const uptime)
{
 ...
 * @cond DEV
 */
idigi_callback_status_t iDigiConnectorClass::app_os_handler(idigi_os_request_t const request,
